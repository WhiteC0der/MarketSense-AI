...
const pulseVariants = {
  unknown: { scale: [1, 1.2, 1], opacity: [1, 0.7, 1] },
};
      ...
      {messages.length === 0 ? (
        <motion.div className="h-full flex items-center justify-center" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }}>
          <div className="text-center">
            <motion.div className="w-12 h-12 rounded-full bg-gradient-to-br from-teal-400 to-cyan-500 flex items-center justify-center mx-auto mb-3" animate={{ y: [0, -8, 0] }} transition={{ duration: 3, repeat: Infinity }}>
        ...
        {isLoading && (
          <motion.div className="flex gap-3" variants={typingVariants} initial="hidden" animate="visible" exit="exit">
            ...
            <div className="flex gap-1.5 items-center">
              {[0, 1, 2].map((i) => (
                <motion.div key={i} className="w-2 h-2 bg-teal-500 rounded-full" animate={pulseVariants.unknown} transition={{ duration: 1.4, repeat: Infinity, delay: i * 0.2 }} />
              ))}
            </div>
          </motion.div>
        )}
